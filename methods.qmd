...
c14_splits <- c14_cleaned %>%
  ...
  #slice(which.CalDates(cal, BP > 2000, p = 0.8)) %>% # focus on earlier dates (1,026 to 663!)
...
ggplot(test_eks5 %>% filter(name == 'tmax'), aes(value, estimate)) +
     geom_line(alpha = 0.01, aes(group = rep))
#ggsave('tmax_arch_1d.png', width = 5, height = 3)

ggplot(test_eks4, aes(twarm, estimate)) +
     geom_line(alpha = 0.01, aes(group = rep))
```
...
ggplot(test_eks2d, aes(MAT, tmax)) +
  geom_contour_ks(aes(group = rep), cont = c(97.5), alpha = 0.1) +
    geom_contour_ks(data = test_eks2d_2, aes(group = rep), cont = c(97.5), alpha = 0.1, color = 'red') +
  coord_equal()
```
