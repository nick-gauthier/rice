...
params %>%
  ...
  geom_linerange(aes(xmin = low_opt, xmax = high_opt), linewidth = 3) +
  theme_bw() +
...
```{r}
bio1 <- rast('data/wc/wc2.1_10m/wc2.1_10m_bio_1.tif')
...
crop <- getCrop('Rice paddy (Japonica)')

library('geodata')
library(here)
tmin <- worldclim_global('tmin', res = 10, path = here('wc'))
tavg <- worldclim_global('tavg', res = 10, path = here('wc'))
prec <- worldclim_global('prec', res = 10, path = here('wc'))
eco <- dismo::ecocrop(crop, tmin, tavg, prec, rainfed = TRUE)
plot(any(is.na(tmin)))

rice <- ecocropPars('Rice paddy (Japonica)')

# create a model
m <- ecocrop(rice)
predict(m, tavg = tavg, prec = prec) %>% plot
plot(eco)
```
