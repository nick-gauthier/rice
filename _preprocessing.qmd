...
```{r}
trace_order <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio01/',#here('data/data-raw/chelsa_trace/bio01'), 
                          full.names = FALSE) %>%
  str_sub(23, -10) %>% 
  as.numeric() %>% 
  sort()
...
trace_tmp <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio01/',#here('data/data-raw/chelsa_trace/bio05'), 
                         full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))

trace_tmax <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio05/',#here('data/data-raw/chelsa_trace/bio05'), 
                         full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))


trace_twarm <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio10/',#here('data/data-raw/chelsa_trace/bio05'), 
                         full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))
...
trace_tmp <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio01/',#here('data/data-raw/chelsa_trace/bio01'), 
                        full.names = TRUE)[trace_order] %>%
...
trace_twarm <- list.files('~/Dropbox (UFL)/Data/CHELSA/chelsa_trace/bio10/',#here('data/data-raw/chelsa_trace/bio10'), 
                         full.names = TRUE)[trace_order] %>%
  read_stars(along = 'time') 
...
trace_ts <- list('bio01', 'bio05', 'bio10') |>
  map(~ paste0(here('data/data-raw/chelsa_trace/'), .x) |>
...
cmip6 <- model_names %>%
  ...
  map_depth(1, ~here('data/data-raw/chelsa_cmip', .x, 'bio') %>% as.list()) %>% #'~/Dropbox (UFL)/Data/CHELSA/CMIP6/2071-2100/'
...
saveRDS(cmip6, here('data/data-derived/cmip6_data.rds'))
```
