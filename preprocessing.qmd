...
This notebook documents the data preprocessing pipeline. The outputs (in `data/derived/`) are included in the repository and used by `main.qmd`.

**To reproduce the main manuscript:** Run `main.qmd` only. All required derived data files are included.

**To re-run preprocessing from scratch requires:**

- \~40 GB: CHELSA-CMIP6 projections
- \~120 GB: CHELSA-TraCE21k paleoclimate data

See the "Data Requirements" section below for download instructions.
...
The following datasets (\~650 MB total) will be automatically downloaded when you run this notebook:

-   CHELSA V2.1 contemporary climate (1981-2010): bio1, bio5, bio10, bio12, PET
-   Erb et al. Holocene temperature reconstruction
-   LGMR (Last Glacial Maximum Reanalysis) paleoclimate data
-   Rice occurrence records: GBIF/herbarium specimens, genetic accessions
-   Archaeological rice data: Multiple published datasets
-   Monfreda et al. global crop extent data

These will be saved to `data/raw/` automatically.

## Manual Download Required

The following datasets must be obtained manually:

### 1. IRRI Rice Extent Map (Required)

-   Reference: Nelson and Gumma (2015)
-   File: asia-rice-extent.tif (\~500 MB)
-   Save to: `data/raw/IRRI Asia Rice Extent Map/`
-   Used for: Main rice extent analysis (Figure 1, Figure 2)
-   Download URL: https://drive.google.com/file/d/0BwTW1oKZU90LTUtqaDQ2XzZpbjg/view?resourcekey=0-cnG60k-2NMgUe7W5hahR5w
-   Note: This dataset is difficult to locate. The above Google Drive link (which downloads the entire directory) was found via the Internet Archive and was functional as of November 2025. If the link becomes unavailable, search the Internet Archive for IRRI rice extent data or contact IRRI directly.

### 2. CHELSA-CMIP6 Future Projections (Required for future scenarios)

-   Source: https://chelsa-climate.org/cmip6/
-   Size: \~40 GB
-   Models needed: GFDL, IPSL, MPI, MRI, UKESM
-   Scenarios needed: SSP126, SSP370, SSP585 (years 2071-2100)
-   Variables needed: bio1, bio5, bio10
-   Save to: `data/raw/chelsa_cmip/{model}/{scenario}/bio/`
-   Used for: Future climate projections (Figures 4-5, Supplementary Figures 6-8)

### 3. CHELSA-TraCE21k Paleoclimate (Optional)

-   Source: https://chelsa-climate.org/chelsa-trace21k/
-   Size: \~120 GB
-   Variables needed: bio01, bio05, bio10 (all 121 time steps)
-   Save to: `data/raw/chelsa_trace/bio01/`, `bio05/`, `bio10/`
-   Used for: Extracting paleoclimate at archaeological sites
-   Note: Optional if using pre-computed arch_trace.rds from data/derived/

## Minimal Reproduction Option

To reproduce the main manuscript without downloading 160 GB of CHELSA data:

1.  Download required datasets: IRRI rice map + CMIP6 projections (\~40 GB)
2.  Use pre-computed files from data/derived/:
    -   arch_trace.rds - Archaeological sites with extracted paleoclimate
    -   trace_boot.rds - Bootstrapped archaeological samples
    -   trace_ts.rds - Holocene temperature time series
3.  Skip or comment out the CHELSA-TraCE21k processing chunks (lines 152-302)

This approach reproduces all figures and analyses except the paleoclimate extraction step (which is already done).
...
├── data/
│   ├── raw/                          # Input data
│   │   ├── IRRI Asia Rice Extent Map/
│   │   │   └── asia-rice-extent.tif  (manual download required)
│   │   ├── chelsa_cmip/              (manual download required)
│   │   │   ├── gfdl/ssp126/bio/
│   │   │   ├── gfdl/ssp370/bio/
│   │   │   └── ... (5 models x 3 scenarios)
│   │   ├── chelsa_trace/             (optional, or use arch_trace.rds)
│   │   │   ├── bio01/ (121 files)
│   │   │   ├── bio05/ (121 files)
│   │   │   └── bio10/ (121 files)
│   │   └── [auto-downloaded files]   (created by this notebook)
│   └── derived/                      # Outputs (created by this notebook)
│       ├── README.md
│       ├── arch_trace.rds            (use this to skip TraCE download)
│       ├── trace_boot.rds
│       └── [other processed files]
```

## Alternative: Streaming CHELSA-TraCE (Experimental)

There is experimental code (lines 212-236) to stream CHELSA-TraCE21k data directly using /vsicurl/ without downloading the full 300 GB. This is slower but avoids the storage requirement. However, it is not yet fully tested for the full workflow.
...
Save lightweight versions for main.qmd:

```{r save-lightweight-chelsa}
# Save cropped and downsampled Asia version for main analysis  
chelsa_asia_cropped <- here('data/raw', filenames[1:3]) |>
  read_stars() |>
  st_crop(asia_bbox_sf) |>
  st_downsample(c(20, 20, 1)) |>
...
saveRDS(chelsa_asia_cropped, here('data/derived/chelsa_asia.rds'))

# Save downsampled world version for supplementary figures
chelsa_world_cropped <- here('data/raw', filenames[1:3]) |>
  read_stars() |>
  st_crop(st_bbox(coasts_world)) |>
  st_downsample(5)

saveRDS(chelsa_world_cropped, here('data/derived/chelsa_world_downsampled.rds'))
```
...
```{r}
#| eval: false
# Load CHELSA-TraCE21k data from local files
# Requires downloading ~300 GB from https://chelsa-climate.org/chelsa-trace21k/
# to data/raw/chelsa_trace/bio01/, bio05/, bio10/

trace_order <- list.files(here('data/raw/chelsa_trace/bio01'),
                          full.names = FALSE) %>%
  str_sub(23, -10) %>%
  as.numeric() %>%
  order()

trace_tmp <- list.files(here('data/raw/chelsa_trace/bio01'),
                        full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))

trace_tmax <- list.files(here('data/raw/chelsa_trace/bio05'),
                         full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))

trace_twarm <- list.files(here('data/raw/chelsa_trace/bio10'),
                          full.names = TRUE)[trace_order] |>
  rast() |>
  crop(ext(asia_bbox_sf))
...
#### Alternative: Streaming from CHELSA (Experimental)

The following function streams CHELSA-TraCE21k data directly from the server using GDAL's `/vsicurl/` protocol. This avoids the 300 GB download but is slower and may timeout for large extractions. Included for reference but not tested for the full workflow.

```{r}
#| eval: false
# EXPERIMENTAL: Stream CHELSA-TraCE21k without downloading
# Slower than local files but avoids 300 GB storage requirement
...
Save the area-averaged Holocene temperature time series for Asia.

```{r}
#| eval: false
# Requires trace data to be loaded (either from local files or streaming)
sf_use_s2(FALSE)
trace_ts <- list('bio01', 'bio05', 'bio10') |>
  map(~ here('data/raw/chelsa_trace', .x) |>
...
cmip6 <- model_names %>%
  ...
  map_depth(1, ~here('data/raw/chelsa_cmip', .x, 'bio') %>% as.list()) %>%
...
cmip6 |>
  ...
  saveRDS(here('data/derived/cmip6_data.rds'))
```
...
Calculate percentage of each country's area exceeding thermal thresholds under current and projected climate scenarios.

```{r}
#| eval: false
# NOTE: Slow to run due to large raster aggregations. Output saved to country_stats.rds.
# Threshold values (28.2, 32.7, 40.4) are 97.5% quantiles calculated in main.qmd.

# Select major rice-producing countries
...
chelsa_obs <- here('data/raw', filenames[1:3]) |>
  read_stars(along = 'variable') |>
  st_crop(asia_bbox_sf) |>
  st_set_dimensions('variable', values = c('bio1', 'bio5', 'bio10'))

# Calculate observed climate exceedance (boolean: does each cell exceed threshold?)
obs_exceedance <- chelsa_obs %>%
  split('variable') %>%
  mutate(bio1 = bio1 > 28.2,
         bio10 = bio10 > 32.7,
         bio5 = bio5 > 40.4) %>%
...
# Calculate CMIP6 projected climate exceedance
cmip6_exceedance <- cmip6 %>%
  split('variable') %>%
  mutate(bio1 = bio1 > 28.2,
         bio10 = bio10 > 32.7,
         bio5 = bio5 > 40.4) %>%
...
# Crop and downsample projected data
cmip6_cropped <- st_crop(cmip6_exceedance, st_bbox(countries_sub)) %>%
  st_downsample(5) %>%
  setNames('sum')

# Calculate cell areas for area-weighted aggregation
cell_areas <- st_area(cmip6_cropped) %>% drop_units()

# Weight exceedance by cell area
cmip6_weighted <- cmip6_cropped * cell_areas
obs_weighted <- obs_exceedance * cell_areas

# Aggregate projected exceedance by country
cmip6_by_country <- aggregate(cmip6_weighted, by = st_geometry(countries_sub), FUN = sum, na.rm = TRUE) %>%
  as_tibble() %>%
  select(-geometry) %>%
  mutate(name = rep(countries_sub$name, length.out = n()))

# Aggregate total area by country
country_areas <- aggregate(cell_areas, by = st_geometry(countries_sub), FUN = sum, na.rm = TRUE) %>%
  as_tibble() %>%
  select(-geometry) %>%
  mutate(name = countries_sub$name)

# Aggregate observed exceedance by country
obs_by_country <- aggregate(obs_weighted, by = st_geometry(countries_sub), FUN = sum, na.rm = TRUE) %>%
  as_tibble() %>%
  select(-geometry) %>%
  mutate(name = rep(countries_sub$name, length.out = n())) %>%
...
country_stats <- left_join(cmip6_by_country, country_areas) %>%
  mutate(percent = round(sum / area * 100)) %>%
  ...
  ungroup() %>%
  bind_rows(obs_by_country)

saveRDS(country_stats, here('data/derived/country_stats.rds'))
```
...
```{r}
set.seed(1234) # for reproducible gridSample operations
...
# Check if IRRI data exists, provide helpful error if not
irri_file <- here('data/raw/IRRI Asia Rice Extent Map/asia-rice-extent.tif')
if (!file.exists(irri_file)) {
  stop("IRRI rice extent map not found. Please download from:\n",
       "https://drive.google.com/file/d/0BwTW1oKZU90LTUtqaDQ2XzZpbjg/view?resourcekey=0-cnG60k-2NMgUe7W5hahR5w\n",
       "and save to: data/raw/IRRI Asia Rice Extent Map/asia-rice-extent.tif\n",
       "See the 'Manual Download Required' section at the top of this notebook for details.")
}
