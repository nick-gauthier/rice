...
irri <- rast('data/data-raw/IRRI Asia Rice Extent Map/asia-rice-extent.tif') %>%
  aggregate(fact = 20) %>%
  mask(., resample(as(chelsa_mask, 'SpatRaster'), .))
...
ggplot() +
  geom_stars(data = st_as_stars(irri) > 0.15) +
  scale_fill_viridis_d() +
  geom_sf(data = coasts, color = 'white') +
    geom_sf(data = sativa, color = 'white', size = .05)
...
cmip6 <- list.files('~/Dropbox (UFL)/CHELSA_CMIP6/ssp585/', full.names = TRUE) %>%
  ...
  setNames(chelsa_names) %>%
  terra::crop(terra::ext(60, 150, -20, 50) ) # global
...
bbox <- raster::extent(-130, 155, -55, 60) 
# 
# bg <- dismo::randomPoints(chelsa_mask, n_background) %>%
#   as_tibble() %>%
...
# thinned <- progen %>%
#   st_coordinates() %>%
#   dismo::gridSample(chelsa_mask) %>%
#     as_tibble() %>%
#     st_as_sf(coords = c('X', 'Y'), crs = 4326) %>% 
#     mutate(presence = 1)
#dat <- bind_rows(thinned, bg) %>%
#  bind_cols(., terra::extract(chelsa, ., ID = FALSE)) %>%
#  mutate(presence = factor(presence, levels = c(1, 0)))

set.seed(1234567)
dat <- spatSample(irri, 10000, na.rm = TRUE, as.points = TRUE) %>%
  st_as_sf() %>%
...
saveRDS(dat, here('data/data-derived/dat.rds'))
```
### Data spliting
...
```{r}
dat <- readRDS(here('data/data-derived/dat.rds')) %>%
  st_drop_geometry()
...
#  mutate(species = as.factor(species)) %>%
    bam(presence ~ s(bio1,  bs = 'cr', k = 5)+ s(bio15, bs = 'cr', k = 5) + s(bio2, bs = 'cr', k = 5) +  s(bio12,  bs = 'cr', k = 5) +  s(bio3,  bs = 'cr', k = 5), data = ., family = binomial(), select = TRUE)
...
ggplot() +
  geom_stars(data = future_pred - pred)  +
  geom_sf(data = coasts %>% st_crop(asia_bbox), color = 'white') +
