...
# actually should include spontaneo here as nivara
rice_list <- c("Oryza sativa", "Oryza rufipogon")
alt_names <- c("Oryza nivara", "Oryza spontanea","oryza sativa", "oryza rufipogon",     "oryza nivara", "oryza nivara & spontanea", "oryza spontanea x nivara", "oryza rufipogon x nivara", "oryza nivara x spontanea","oryza nivara x rufipogon", "Oryza sativa L.", "oryza spontanea x rufipogon","oryza rufipogon x spontanea","oryza rufipogon x nivara x spontanea","oryza rufipogon GRIFF.","oryza nivara & rufipogon", "oryza nivara SHARMA ET SHASTRY", "oryza sativa LINNAEUS", "Oryza sativa L., 1753")

library(CoordinateCleaner)
oryza_raw <- read_tsv('data/data-raw/0266607-220831081235567.csv') %>%
  filter_na(species %in% rice_list | verbatimScientificName %in% alt_names) %>%
  filter_na(!(scientificName %in% c("Oryza glumipatula Steud.", 
                                      "Oryza glumaepatula Steud."))) %>%
  filter_na(!str_detect(verbatimScientificName, 'perennis')) %>%
  filter_na(establishmentMeans != 'Uncertain') %>%
  filter_na(str_detect(issue, 'COORDINATE_ROUNDED', negate = TRUE)) %>%
  filter_na(str_detect(issue, 'BASIS_OF_RECORD_INVALID', negate = TRUE)) %>% 
  mutate(newspecies = if_else(str_detect(verbatimScientificName, 'sativa'), 
                              'Oryza sativa', 'Oryza rufipogon'),
         species = if_else(is.na(species), newspecies, species)) %>%
  select(species, decimallongitude = decimalLongitude, 
...
turt <- read_tsv('data/data-raw/0266607-220831081235567.csv') %>% filter(is.na(species)) %>% filter(verbatimScientificName %in% alt_names)  %>% st_as_sf(coords = c('decimalLongitude', 'decimalLatitude'), crs = 4326) %>%
  st_crop(asia_bbox)

oryza_clean <- clean_coordinates(x = oryza_raw, inst_rad = 5000, outliers_td = 100)
summary(oryza_clean)
plot(oryza_clean)
  
oryza <- oryza_raw[oryza_clean$.summary,] %>%
  filter(!(species == 'Oryza rufipogon' & between(decimallongitude, 0, 20)),# remove obs in africa
         !(species == 'Oryza rufipogon' & decimallatitude > 30),
         !(species == 'Oryza officinalis' & decimallatitude > 30),
          !(species == 'Oryza glaberrima' & decimallongitude > 20),
                   !(species == 'Oryza sativa' & decimallatitude > 60),
                            !(species == 'Oryza sativa' & decimallatitude < -30),
                                     !(species == 'Oryza sativa' & decimallatitude > 35 & between(decimallongitude, 100, 120))) %>% 
...
rnaturalearth::ne_states(country = 'China', returnclass = 'sf') %>%
  ggplot() +
  geom_sf() +
  geom_sf(data = oryza_sf %>% filter(species == 'Oryza sativa'), size = .5) +
#  geom_sf(data = domestic %>% st_crop(asia_bbox), color = 'red') +
  geom_sf(data = turt, color = 'yellow', alpha = .5, size = .5)
```

```{r}
oryza_sf %>%
  mutate(year_test = case_when(is.na(year) ~ 'no year',
                               year < 1970 ~ 'old',
                               year > 2010 ~ 'new',
                               TRUE ~ 'ok')) %>%
  filter(species != 'Oryza sativa') %>%
ggplot() +
  geom_sf( size = .5, aes(color = species), alpha = 0.8)+
  geom_sf(data = coasts) +
  scale_color_brewer(palette = 'Spectral') +
  facet_wrap(~year_test, nrow = 3)
```
...
oryza_sf %>%
  ...
  filter(species == 'Oryza sativa') %>%
ggplot() +
  geom_sf( size = .5, alpha = 0.8) +
  geom_sf(data = coasts) +
  scale_color_brewer(palette = 'Spectral') +
  facet_wrap(~year_test, nrow = 3)
```

What to do about the no years? Its clearly where the most erroneous points are but it also has good coverage of china. need to find a way to filter it futher.
...
oryza_sf %>%
  filter(is.na(year)) %>% 
  ggplot(aes(color = institutionCode)) +
  geom_sf()

oryza_sf %>% filter(institutionCode == 'Bioversity-SINGER') %>%
...
```{r}
ggplot(oryza_sf %>% filter(!is.na(establishmentMeans))) +
  geom_sf( size = .7, aes(color = establishmentMeans), alpha = .5)+
  geom_sf(data = coasts)
```

```{r}
oryza %>%
  filter(establishmentMeans == 'Uncertain') %>%
  pull(issue)
```

```{r}
oryza %>%
  filter(species == 'Oryza rufipogon') %>% 
  filter(st_coordinates(geometry)[,2] > 35.5) %>%
  plot
```
...
library(CoordinateCleaner)
rl <- clean_coordinates(x = oryza_sf %>% mutate(decimallatitude = st_coordinates(geometry)[,2],
                                             decimallongitude = st_coordinates(geometry)[,1]) %>% st_drop_geometry(),inst_rad = 5000, outliers_td = 100)
summary(rl)
plot(rl)
```

```{r}
dat_cl <- oryza_sf[rl$.summary,]
```
...
dat_cl %>%   
  ...
  dat_cl %>%   
  st_as_sf(coords = c('decimalLongitude', 'decimalLatitude'), crs = 4326) %>%
  filter(str_detect(issue, 'BASIS_OF_RECORD_INVALID')) %>%
  ggplot() +
    geom_sf(aes(color = species)) +
    geom_sf(data = coasts) +
  facet_wrap(~species)
   ...
   dat_cl %>%   
  st_as_sf(coords = c('decimalLongitude', 'decimalLatitude'), crs = 4326) %>%
  filter(str_detect(issue, 'COORDINATE_ROUNDED|BASIS_OF_RECORD_INVALID'),negate = TRUE) %>%
  ggplot() +
    geom_sf(aes(color = species)) +
    geom_sf(data = coasts) +
       facet_wrap(~species)
  ...
  ggplot() +
    ...
    geom_sf(data = coasts) +
       facet_wrap(~species)
        ...
        dat_cl %>%   
  st_as_sf(coords = c('decimalLongitude', 'decimalLatitude'), crs = 4326) %>%
  filter(str_detect(issue, 'COORDINATE_ROUNDED', negate = TRUE) )%>%
  #        filter(str_detect(issue, 'BASIS_OF_RECORD_INVALID')) %>%
  ggplot() +
    geom_sf(aes(color = species)) +
    geom_sf(data = coasts) +
...
```{r}
oryza_cln_dat <- read_tsv('data/data-raw/0266607-220831081235567.csv') %>%
      filter_na(species %in% rice_list | verbatimScientificName %in% tolower(rice_list)) %>%
    mutate(species = if_else(is.na(species), stringr::str_to_sentence(verbatimScientificName), species)) %>%
    filter_na(establishmentMeans != 'Uncertain') %>%
      filter_na(str_detect(issue, 'COORDINATE_ROUNDED', negate = TRUE)) %>%
      filter_na(str_detect(issue, 'BASIS_OF_RECORD_INVALID', negate = TRUE)) %>%
...
rl <- clean_coordinates(x = oryza_cln_dat, centroids_rad = 50000, centroids_detail = 'country', inst_rad = 20000, outliers_td = 100)
summary(rl)
plot(rl)
  
dat_cl2 <- oryza_cln_dat[rl$.summary,] %>%
  filter(!(species == 'Oryza rufipogon' & between(decimallongitude, 0, 20)),# remove obs in africa
         !(species == 'Oryza rufipogon' & decimallatitude > 30),
         !(species == 'Oryza officinalis' & decimallatitude > 30),
          !(species == 'Oryza glaberrima' & decimallongitude > 20),
...
```{r}
saveRDS(dat_cl2, 'data/data-derived/oryza_clean.rds')
```
...
```         
bio15          bio2           bio3           npp            pet_penman_min rsds_min       rsds_range  
```
...
```{r}
test <- predict(c(chelsa), small_fit, cores = 10, na.rm = TRUE)
```

```{r}
dat2 <- prep(base_rec) %>% juice
bart_test <- embarcadero::variable.step(dat2[,-37], -1 * as.numeric(dat2$presence) + 2, iter = 20)
```
...
PC2 is max temp warmestr month and quarter positive and negative are basically summer rainfall indicators.

PC3 is precip seasonality and wetness indicators
...
```{r}
erb_rotated <- read_ncdf('~/Downloads/holocene_reconstruction.nc', var = 'recon_tas_mean') %>%
  st_set_crs(4326)
...
as('SpatRaster') %>%
  rotate() %>%
  tapp(centuries, fun = mean) %>%
  st_as_stars(crs = 4326) %>%
  st_set_dimensions('band', names = 'time', values = unique(centuries) / 1000 - 1) %>%
  setNames('tas') %>%
  st_crop(bbox) %>%
...
as('SpatRaster') %>%
  rotate() %>%
  st_as_stars(crs = 4326) %>%
  st_set_dimensions('band', names = 'time', values = -1 * (times + 0.5)) %>%
  setNames('tas') %>%
 st_crop(bbox) %>%
  mutate(tas = units::set_units(tas, degree_c))
  ...
  ggplot() +
  geom_stars(data = erb %>% filter(time > -7))+ 
  facet_wrap(~time) +
  scale_fill_distiller(palette = 'RdBu', limits = c(-.5, .5)) +
  geom_sf(data = coasts %>% st_crop(bbox)) +
    theme_minimal()
...
arch %>%
    filter(age > -10000) %>%
  select(age) %>%
  st_extract(erb_decadal, ., time_column = 'age', interpolate_time = TRUE) %>%
  ggplot(aes(age, tas)) +
  geom_point()
...
```{r}
nh_mean <- read_ncdf('data/data-raw/holocene_reconstruction.nc', 
                     var = 'recon_tas_nh_mean') %>%
  as_tibble()
  
ggplot(nh_mean,aes(ages, recon_tas_nh_mean)) +
  geom_line(alpha = 0.01, aes(group = ens)) +
  geom_smooth()
```
...
age_gam <- bind_cols(arch, st_coordinates(arch)) %>%
  rename(x =X, y = Y) %>%
  select(age, x, y) %>%
  filter(age > -10000) %>%
  gam(age ~ s(x, y), data = .)
