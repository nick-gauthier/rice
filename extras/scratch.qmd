...
rice_list <- c("Oryza sativa", "Oryza rufipogon","Oryza officinalis","Oryza barthii" ,"Oryza glaberrima","Oryza nivara")
oryza <- read_tsv('data/data-raw/0266607-220831081235567.csv') %>%
...
summary(oryza)
ggplot() +
  geom_sf(data = oryza, size = .1, aes(color = year))
...
oryza %>%
  ...
  filter(verbatimScientificName %in% tolower(rice_list)) %>%
  plot
geodata::sp_genus('Oryza')
...
geodata::sp_genus('Oryza')
geodata::sp_occurrence('Oryza', 'glumaepatula', download = FALSE, removeZeros = TRUE)
```
...
```{r}
ggplot(oryza %>% filter(!is.na(establishmentMeans))) +
...
rl <- clean_coordinates(x = oryza %>% mutate(decimallatitude = st_coordinates(geometry)[,2],
                                             decimallongitude = st_coordinates(geometry)[,1]) %>% st_drop_geometry(), centroids_rad = 40000, centroids_detail = 'country', inst_rad = 20000, outliers_td = 100)
summary(rl)
plot(rl)
```

```{r}
dat_cl <- oryza[rl$.summary,]
```
...
rice <- readRDS( 'data/data-derived/oryza_clean.rds')

ggplot() +
  geom_sf(data = rice, aes(color = species)) +
  facet_wrap(~species) +
  geom_sf(data = coasts)
```
...
xvars <- names(train)[!(names(train) == 'presence')]
## Run the BART model
sdm <- bart(y.train = as.numeric(as.character(train$presence)),
 x.train=train[,xvars], 
 keeptrees = TRUE)
...
map <- predict(sdm, as(chelsa, 'Raster'))
```

```{r cache = TRUE}
sdm_step <- bart.step(x.data = train[,xvars], 
                      y.data = as.numeric(as.character(train$presence)),
                      full = FALSE)
```
...
sdm_simp <- bart(y.train = as.numeric(as.character(train$presence)), 
 x.train=train[,c('bio10',  'bio11')], 
 keeptrees = TRUE)

partial(sdm_simp, 
        x.vars = c('bio10',  'bio11'),
    ci = TRUE,
    panels = TRUE,
    smooth = 3)
```
